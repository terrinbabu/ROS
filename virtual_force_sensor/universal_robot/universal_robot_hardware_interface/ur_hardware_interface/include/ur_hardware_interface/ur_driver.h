...
public:
	...
	UrDriver(std::condition_variable& rt_msg_cond, std::condition_variable& msg_cond, std::__cxx11::string host, unsigned int reverse_port = 50007, unsigned int safety_count_max = 12, double min_payload = 0., double max_payload = 1., double speedj_timeout=0.016);
	bool start();
	void halt();
