...
UrCommunication::UrCommunication(std::condition_variable& msg_cond,
	...
	if (pri_sockfd_ < 0) {
		ROS_FATAL("ERROR opening socket pri_sockfd");
	}
	sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
	if (sec_sockfd_ < 0) {
		ROS_FATAL("ERROR opening socket sec_sockfd");
	}
	server_ = gethostbyname(host.c_str());
	if (server_ == NULL) {
		ROS_FATAL("ERROR, unknown host");
	}
...
bool UrCommunication::start() {
  ...
  ROS_DEBUG("Firmware version detected: %.7f", robot_state_->getVersion());
	close(pri_sockfd_);
		...
		if (keepalive_) {
			...
			while (keepalive_ && !connected_) {
				...
				if (flag_ < 0) {
					...
					connected_ = true;
					ROS_INFO("Secondary port: Reconnected");
				}
			}
		}
	}
