...
UrDriver::UrDriver(std::condition_variable& rt_msg_cond,
		...
		unsigned int reverse_port, unsigned int safety_count_max, double min_payload, double max_payload, double speedj_timeout) 
		
{
  ...
  minimum_payload_=min_payload;
	...
	rt_interface_ = new UrRealtimeCommunication(rt_msg_cond, host,
                                              safety_count_max, speedj_timeout);
	new_sockfd_ = -1;
...
{
  rt_interface_->setPosition(q0, q1, q2, q3, q4, q5, dt);
}
